...
module gesture_uart_bridge_tb;
    ...
    task hold_then_release_right;
        begin
            @(negedge clk); tilt_right_level = 1'b1;
            repeat (16) @(posedge clk);
            @(negedge clk); tilt_right_level = 1'b0;
        end
    endtask

    task hold_then_release_forward;
        begin
            @(negedge clk); tilt_forward_level = 1'b1;
            repeat (16) @(posedge clk);
            @(negedge clk); tilt_forward_level = 1'b0;
        end
    endtask
    ...
    task hold_then_release_backward;
        begin
            ...
            repeat (16) @(posedge clk);
            @(negedge clk); tilt_backward_level = 1'b0;
        end
    endtask
    ...
    initial begin
        ...
        fork hold_then_release_right(); expect_uart_byte("R"); join
        wait_for_full_rearm();

        fork hold_then_release_forward(); expect_uart_byte("U"); join
        wait_for_full_rearm();

        fork hold_then_release_backward(); expect_uart_byte("D"); join
        wait_for_full_rearm();

        fork pulse_tap(); expect_uart_byte("T"); join
        wait_for_full_rearm();

        before_count = command_count;
        fork shake_with_tail_chatter(); expect_uart_byte("S"); join
        wait_for_full_rearm();
        if (command_count !== before_count + 1) begin
        ...
        before_count = command_count;
        fork flip_with_tail_chatter(); expect_uart_byte("F"); join
        wait_for_full_rearm();
        if (command_count !== before_count + 1) begin
        ...
        before_count = command_count;
        fork tap_then_shake(); expect_uart_byte("S"); join
        wait_for_full_rearm();
        if (command_count !== before_count + 1) begin
        ...
        before_count = command_count;
        fork backward_then_flip_candidate(); expect_uart_byte("F"); join
        wait_for_full_rearm();
        if (command_count !== before_count + 1) begin
            $display("FAIL: flip candidate did not suppress backward cross-trigger");
            $fatal;
        end
